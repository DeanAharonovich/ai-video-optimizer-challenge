...
async function seedDatabase() {
  ...
  const runningTest = await storage.createTest({
    ...
    variants: [
      ...
      },
      {
        ...
        thumbnailUrl: "https://images.unsplash.com/photo-1523381294911-8d3cead23475?w=800&q=80",
  ...
  // Generate analytics data for the running test (last 7 days)
  const now = new Date();
  const analyticsData = [];
  ...
  for (let i = 0; i < 168; i++) { // 7 days of hourly data
    ...
    // Variant B - Better performance
    analyticsData.push({
      testId: runningTest.id,
      variantId: runningTest.variants[1].id,
      timestamp,
      views: Math.floor(Math.random() * 50) + 100,
      conversions: Math.floor(Math.random() * 8) + 4, // Higher conversions
      interactions: Math.floor(Math.random() * 15) + 8, // Higher interactions
    });
  }
